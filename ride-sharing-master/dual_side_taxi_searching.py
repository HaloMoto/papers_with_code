...
for grid_cell in Gt[G_origin-1]:
    if new_query.curr_time + T[int(grid_cell)][int(G_origin-1)] < new_query.curr_time + time_window:
...
for grid_cell in Gt[G_destination-1]:
    if new_query.curr_time + T[int(grid_cell-1)][int(G_destination-1)] < T[int(G_origin-1)][int(G_destination-1)] + time_window :
...
for taxi in Tlist[G_origin-1]:
    if TimeTlist[int(G_origin-1)][int(taxi)] >= new_query.curr_time and taxi < new_query.curr_time + time_window :
        S_origin.add(taxi)
    else:
        break
            
for taxi in Tlist[G_destination-1]:
    if TimeTlist[int(G_destination-1)][int(taxi)] >= new_query.curr_time and taxi < T[int(G_origin-1)][int(G_destination-1)] + time_window :
...
while len(S_origin | S_destination) == 0 and (Stop_O == False or Stop_D == False) :
    ...
    for taxi in Tlist[int(l_d[i]-1)]:
        if TimeTlist[int(l_d[i]-1)][int(taxi)] >= new_query.curr_time and taxi < T[int(G_origin-1)][int(G_destination-1)] + time_window - T[int(l_d[i]-1)][int(G_destination-1)]:
            S_destination.add(taxi)
        else:
            break
    print(S_origin | S_destination)
