...
## 拼单算法
def combination_of_multiple_orders(query_list, regl_Hexg_grids, driver_list, t_cur, time_slot):
    ...
    for key in dict_of_delivery_point_belongs_to.keys():
        ...
        if len(dict_of_delivery_point_belongs_to[key]) >= 2 and len(dict_of_delivery_point_belongs_to[key]) <= 4:
            ...
            # 上下班高峰期兼职司机接客
            if True:
                driver_list[0].number_of_order += len(merged_order)
                is_successful = True
            else:
            ...
            for i in range(k):
                ...
                if len(result_of_classifier[i]) >= 2 and len(result_of_classifier[i]) <= 4:
                    ...
                    for j in range(num_of_drivers):
                        ...
                        # 上下班高峰期兼职司机接客
                        if time_slot == 6:
                            driver_list[0].number_of_order += len(merged_orders[j])
                            is_successful = True
                        else:
