...
class Query:
    def __init__(self, query_id, pickup_location, current_time):
        ...
        self.latest_pickup_time = current_time + datetime.timedelta(seconds=10)
        self.latest_delivery_time = self.latest_pickup_time + datetime.timedelta(seconds=T[self.pickup_location-1][self.delivery_location-1]) + datetime.timedelta(seconds=10)
