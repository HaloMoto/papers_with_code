    ...
    for i in range(int(num_of_querys)):
        ...
        for j in range(len(hot_index)-1):
            if temp > hot_index[j] and temp <= hot_index[j+1]:
                ...
                q = Query(query_id, j-1+245, starttime)
                query_total_distance += D[q.pickup_location-1][q.delivery_location-1]
                query_list.append(q)
                query_id += 1
                break
...
print("Query total distance:",query_total_distance)
