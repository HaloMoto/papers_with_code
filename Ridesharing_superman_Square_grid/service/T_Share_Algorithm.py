...
## 拼单算法
def combination_of_multiple_orders(query_list, sqr_grids, driver_list, t_cur):
    ...
    for query in query_list:
        for i in sqr_grids[query.delivery_location-num_of_intersections-1].delivery_cluster:
            # print("=======================")
    ...
    for key in dict_of_delivery_point_belongs_to.keys():
        ...
        if len(dict_of_delivery_point_belongs_to[key]) >= 2 and len(dict_of_delivery_point_belongs_to[key]) <= 4:
            ...
            ## 将合并的订单分配给司机
            # 调用搜索车辆算法，返回成功 与否状态
            is_successful = find_the_nearest_empty_car_for_many(sqr_grids, merged_order, driver_list, t_cur)
            ## 如果找到了空车，那么改订单状态为已服务
            if is_successful:
                ## test ##
                print("拼单成功1")
            ...
            for i in range(k):
                ...
                if len(result_of_classifier[i]) >= 2 and len(result_of_classifier[i]) <= 4:
                    ...
                    for j in range(num_of_drivers):
                        ...
                        ## 将合并的订单分配给司机
                        # 调用搜索车辆算法，返回成功 与否状态
                        is_successful = find_the_nearest_empty_car_for_many(sqr_grids, merged_orders[j], driver_list, t_cur)
                        ## 如果找到了空车，那么改订单状态为已服务
                        if is_successful:
                            ## test ##
                            print("拼单成功3")
...
## 订单插入检查 ##
def insertion_feasibility_check(query, driver, m, n, t_cur, sqr_grids):
    ...
    if t_cur + datetime.timedelta(seconds=t_j+driver.assist_t) > query.latest_delivery_time:
        ...
        for node in driver.route:
            sqr_grids[nodes_belong_to_which_grid[int(node)]-1].driver_will_coming = [driver_tuple for driver_tuple in sqr_grids[nodes_belong_to_which_grid[int(node)]-1].driver_will_coming if driver_tuple[0] != driver.driver_id]
        # 重新计算路径
        driver.route = []
        driver.route.extend(
    ...
    # 从格子的driver_will_coming中删除剩下路径
    for node in driver.route:
        sqr_grids[nodes_belong_to_which_grid[int(node)] - 1].driver_will_coming = [driver_tuple for driver_tuple in
                                                                                    sqr_grids[
...
## 为合单寻找一辆空车
def find_the_nearest_empty_car_for_many(sqr_grids, merged_order, driver_list, t_cur):
    ...
    if not S_intersection:
        ...
        for grid in S_intersection:
            ...
            # 将浮点数转换为整数
            grid = int(grid)
            for driver_tuple in sqr_grids[grid-1].driver_will_coming:
                ## test ##
                # print("将要到的司机",sqr_grids[grid-1].driver_will_coming)
                ## test ##
                if driver_list[driver_tuple[0]].num_of_occupied_position == 0 and driver_tuple[1] < merged_order[0].latest_pickup_time:
                    ...
                    for node in driver_list[driver_tuple[0]].route:
                        sqr_grids[nodes_belong_to_which_grid[int(node)] - 1].driver_will_coming = [driver_tuple for
                                                                                                    driver_tuple in
                                                                                                    sqr_grids[
                                                                                                        nodes_belong_to_which_grid[
                                                                                                            int(node)] - 1].driver_will_coming
...
## 双边查询算法
def dual_side_taxi_searching(driver_list, sqr_grids, query, t_cur):
    # 订单接应点
    g_o = query.pickup_location
    # 接应点所在格子内的所有司机，并从这些司机中筛选出在pickup_latest_time之前到这个格子的司机
    S_o = []
    for driver_tuple in sqr_grids[query.pickup_location-num_of_intersections-1].driver_will_coming:
    ...
    S_d = []
    for driver_tuple in sqr_grids[query.delivery_location-num_of_intersections-1].driver_will_coming:
    ...
    while not S_intersection and (stop_o == False or stop_d == False):
        if l_o:
            ...
            g_i = l_o.pop(0)
            ## test ##
            # print("helo",sqr_grids[int(g_i)-1].driver_will_coming)
            ## test ##
            # 将格子中的司机取出来
            for driver_tuple in sqr_grids[int(g_i) - 1].driver_will_coming:
                if driver_tuple[1] <= query.latest_pickup_time and driver_list[driver_tuple[0]].num_of_occupied_position != 0 and driver_list[driver_tuple[0]].num_of_occupied_position < 4:
                    S_o.append(driver_list[driver_tuple[0]])
        else:
            stop_o = True
        if l_d:
            ...
            g_j = l_d.pop(0)
            # 将格子中的司机取出来
            for driver_tuple in sqr_grids[int(g_j)-1].driver_will_coming:
                if driver_tuple[1] <= query.latest_delivery_time and driver_list[driver_tuple[0]].num_of_occupied_position != 0 and driver_list[driver_tuple[0]].num_of_occupied_position < 4:
                    S_d.append(driver_list[driver_tuple[0]])
        else:
            stop_d = True
    ...
    if S_intersection:
        for j in range(len(S_intersection)):
            for m in range(1, int(len(S_intersection[j].cur_schedule))+1):
                for n in range(m, int(len(S_intersection[j].cur_schedule))+2):
                    if insertion_feasibility_check(query, S_intersection[j], m, n, t_cur, sqr_grids):
                        ## test ##
                        print("插入成功")
                        if query.condition == 1:
                            print("错误2")
        ...
        # 如果查询结果为空
        if find_the_nearest_empty_car_for_one(query,driver_list,sqr_grids,t_cur):
            query.condition = 1
            print("空车司机加一")

## 推荐算法
def recommendation(driver_list, pickup_clusters, t_cur, sqr_grids, amplification_factor):
    ## 预测模型参数
    a = 1
    b = 3/4
    c = 2/4
    ...
    for driver in driver_list:
        ...
        if driver.num_of_occupied_position == 0 and not driver.route:
            ## 初始化变量
            ## 寻找到距离司机最近的区域
            nearest_cluster_id = sqr_grids[nodes_belong_to_which_grid[driver.cur_location]-1].pickup_cluster[0]
            ## 到达最近区域所需要的时间
            ...
            for node_will_pass in driver.route:
                node_will_pass = int(node_will_pass)
                sqr_grids[nodes_belong_to_which_grid[node_will_pass] - 1].driver_will_coming.append([driver.driver_id, t_cur + datetime.timedelta(seconds=T[driver.cur_location - 1][node_will_pass - 1])])

## 为一单寻找一个空车司机
def find_the_nearest_empty_car_for_one(query, driver_list, sqr_grids, t_cur):
    ## 在订单的接载点所在格子寻找空车
    g_o = query.pickup_location
